<template>
    <div>
      <h1>Propuesta {{ propuestaId }}</h1>
      <div class="espacio"></div>
  
      <div class="propuesta" v-if="propuesta">
        <h2>{{ propuesta.titulo }}</h2>
        <div class="espacio"></div>
        <p>Propuesta: {{ propuesta.propuesta }}</p>
      </div>
  ...
  async function cargarPropuesta() {
    const { data, error } = await supabase
      .from('propuestas')
      .select('titulo, propuesta')
      .eq('id', propuestaId)
      .single();
  
    if (error) {
      console.error('Error cargando la propuesta:', error.message);
      return;
    }
